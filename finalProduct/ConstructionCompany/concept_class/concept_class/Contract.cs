    ...
    public partial class Contract : Form
    {
        ...
        private void toolStripButton2_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                lbl_totalamount.Text = StrQry.ToString();
                decimal rem = Convert.ToDecimal(txt_quantity.Text) * Convert.ToDecimal(txt_unit_qty.Text) * Convert.ToDecimal(txt_amount.Text);
                StrQuery = "update tbl_contract set contrac_Name = '" + txt_con_name.Text.Trim().ToString() + "',contrac_quantity = '" + txt_quantity.Text.Trim().ToString() + "',contrac_unit = '" + txt_unit.SelectedItem.ToString() + "',contrac_unit_quantity = '" + txt_unit_qty.Text.Trim().ToString() + "',contrac_amount = '" + txt_amount.Text.Trim().ToString() + "',contrac_remakrs = '" + rem.ToString() + "' where contrac_ID = '" + txt_id.Text.Trim().ToString() + "'";
                tea.simInsert(StrQuery);
